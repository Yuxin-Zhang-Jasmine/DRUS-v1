...
%% Computing svd(CBH)
%-----------------------
CBH = C * single(full(B*H));
clearvars -except CBH

%% 5 hours
tic; [Ud,Sigma,Vdmatlab] = svd(CBH, 'vector'); toc
pause(240); 
clearvars CBH
pause(240); save Ud.mat Ud -mat;      clearvars Ud
pause(60); save Sigma.mat Sigma -mat; clearvars Sigma

% Permutation the rows of V to fit in python
permutation = [1:256*256];
permutation = reshape(permutation, 256,256);
permutation = permutation';
permutation = permutation(:);
Vd = Vdmatlab(permutation,:); pause(60); save Vd.mat Vd -mat; 
