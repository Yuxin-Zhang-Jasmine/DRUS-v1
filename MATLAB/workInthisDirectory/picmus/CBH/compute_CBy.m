...
[dataset,dasSaveName] = datasetReader(3,4,2);  
...
yPICMUS = yPICMUS .* 10;     % simu_reso=10, simu_cont=30, expe_reso=10, expe_cont=30
...
figure; 
subplot('221');Image_realScale(By,string(['Rx #f = ', string(rx_f_number)]), scan)
subplot('222');plot(yPICMUS(k:end,:)); title('channel data');xlabel('sample index');
subplot('223');Image_realScale(CBy,['CHty img'], scan)
subplot('224');plot(CBy); title('CHty data');

% -- save CBy
save(['picmus/CBH/yd/' dasSaveName], "CBy");
