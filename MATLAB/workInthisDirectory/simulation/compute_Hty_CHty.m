...
load('simulation/svd/V.mat')
load('simulation/svd/lbd.mat')
C = V' ./ lbd;

% ------------------------------------------------
%% DDRM Input Builder (Simulation of 3 channels) 
%--------------------------------------------------
% Param
close all
...
path = ['simulation/SimulationResults/' folderName '/' num2str(naturalImgIdx) '/'];
try 
    load([path 'o_orig_' num2str(naturalImgIdx) '.mat'])
catch
    warning('o_orig does not exist. Load orig and create o_orig...');

    load([path 'orig_' num2str(naturalImgIdx) '.mat'])
    ...
    x_orig = x_orig/max(max(x_orig));
    save([path 'o_orig_' num2str(naturalImgIdx) '.mat'], 'x_orig');
    %return
end
o1 = double(x_orig(:));
o2 = double(x_orig(:));
...
for std_noisey = [0.3, 0.7, 1, 1.5, 2, 2.5, 3, 3.5] %gamma levels  
    ...
    % --save yd
    o_Hty = o_Hty(:);
    save([path, 'yd/', saveNamePre, '_Hty_',  num2str(naturalImgIdx),'.mat'], 'o_Hty');
    save([path, 'yd/', saveNamePre, '_CHty_', num2str(naturalImgIdx), '.mat'], 'o_CHty');
    'saved'
    pause(1);
end