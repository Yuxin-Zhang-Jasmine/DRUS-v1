...
clearvars H_right H_left 


% ------------------------------------------
%% SVD (use EigenValue decomposition)
%--------------------------------------------
tic;   HtH = Ht * H;  toc

clearvars -except HtH
HtH = single(full(HtH));


%% 92 mins eig(HtH)
tic;  [V,LBD] = eig(HtH, 'vector');  toc
clearvars HtH
pause(60); 
...
% Permutating the rows of V for python
permutation = [1:256*256];
permutation = reshape(permutation, 256,256);
permutation = permutation';
permutation = permutation(:);
Vp = V(permutation,:); pause(60); save Vp.mat Vp -mat; 


% take the square root of LBD for CHtH model
